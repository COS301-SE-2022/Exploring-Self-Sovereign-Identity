...
const usrCredentials : OrganizationCredentials = new OrganizationCredentials("ssi", usrCredentialsData);
const creds : OrganizationCredentials[] = [usrCredentials];

const userData : UserData = new UserData("#fxxy", "ssiUsr01", 1, usrAttributes, creds);
...
describe("ContractService", () => {
    ...
    test("contract attribute values should be updated if exist on user data", () => {
        ...
        expect(contract.getAttributes().find(a => {
            a.getName() == "name";
        })?.getValue()).toBe(userData.getAttributes().find(a => {
            a.getName() == "name;"
        })?.getValue());
    });
